...
public class AdminControllerTest {
    ...
    public void getUserOrdersByEmail() throws Exception {
        mockMvc.perform(get(PathConstants.API_V1_ADMIN + PathConstants.ORDER_BY_EMAIL, USER_EMAIL)
                        .contentType(MediaType.APPLICATION_JSON_VALUE))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[*].id").isNotEmpty())
                .andExpect(jsonPath("$[*].totalPrice", hasItem(TOTAL_PRICE)))
    ...
    public void getUserById() throws Exception {
        mockMvc.perform(get(PathConstants.API_V1_ADMIN + PathConstants.USER_BY_ID, USER_ID)
                        .contentType(MediaType.APPLICATION_JSON_VALUE))
                .andExpect(status().isOk())
